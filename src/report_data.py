...
def get_service_report_data(feed_dir: str, service_id: str, trips: List[TripLine], date: str, stops_for_trips: Dict[str, List[StopTime]], stops: Dict[str, Any] = None) -> Dict[str, Any]:
    ...
    for trip in trips:
        ...
        distance_km = None
        if first_stop.shape_dist_traveled is not None and last_stop.shape_dist_traveled is not None:
    ...
    result = {
        ...
        "date": date,
        "trip_rows": trip_rows,
        "total_distance": f"{total_distance_km:,.2f}" if total_distance_km > 0 else None,
